...
Concorrência e atomicidade (G3, achados Muse F1/F4): o limite `max_proposals` é aplicado dentro de uma transação que bloqueia a linha do pedido (`SELECT ... FOR UPDATE` no PostgreSQL), revalida o estado e insere a proposta + incrementa `proposals_received` na mesma transação. Dois parceiros **distintos** propondo ao mesmo tempo na última vaga produzem exatamente `201` + `400 emergency_not_accepting_proposals`, com 1 linha e contador 1. Se o incremento falhar, a proposta sofre rollback (nada é gravado e nenhuma notificação é enviada). A semântica numérica de `max_proposals` não mudou: o pedido é criado com o valor de `guincho_max_proposals_per_request` (default 5 quando a setting não existe). Retirada (`withdraw`) **não** decrementa `proposals_received`: o contador é histórico e a vaga não reabre (decisão de produto pendente — ver `docs/gauntlet/G3-MUSE-FINDINGS-RESOLUTION.md`, F3).

O `GET /api/emergency-requests/nearby?type=tow` lista apenas oportunidades reais (`pending`, `awaiting_proposals`, deadline futuro, ordenadas por distância); usa `latitude`/`longitude` explícitas em par válido ou o cadastro do parceiro (`partners.latitude`/`longitude`). Coordenadas ausentes/inválidas/`0,0` explícitas retornam `400 invalid_coordinates`; par explícito com string vazia ou só espaços (`latitude=&longitude=`) também é `400 invalid_coordinates` e **não** cai para o cadastro; admin sem par explícito retorna `400 coordinates_required`; cadastro de parceiro sem coordenadas retorna `400 partner_onboarding_required`; `radius` precisa ser finito `> 0` (`400 invalid_radius`); `exclude_proposed=true` remove apenas o pedido em que o próprio parceiro tem proposta `pending`. Pedidos legados com coordenada nula, fora dos limites ou `0,0` são ignorados pelo `type=tow` (não aparecem nem derrubam a listagem com 500).
