...
class TowProposalService {
  ...
  static Future<List<TowProposal>> getEmergencyProposals(String emergencyRequestId) async {
    try {
      final headers = await _authHeaders;
      
      final response = await http.get(
        Uri.parse('$baseUrl/api/tow-proposals/emergency/$emergencyRequestId'),
        headers: headers,
      );
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Falha ao buscar propostas: ${response.body}');
      }
    } catch (e) {
      throw Exception('Erro ao buscar propostas: $e');
    }
  }
  ...
  // Aceitar proposta (para cliente)
  static Future<TowProposal> acceptProposal(String proposalId) async {
    try {
      final headers = await _authHeaders;
      
      final response = await http.post(
        Uri.parse('$baseUrl/api/tow-proposals/$proposalId/accept'),
        headers: headers,
      );
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Falha ao aceitar proposta: ${response.body}');
      }
    } catch (e) {
  ...
  // Rejeitar proposta (para cliente)
  static Future<TowProposal> rejectProposal(String proposalId) async {
    try {
      final headers = await _authHeaders;
      
      final response = await http.post(
        Uri.parse('$baseUrl/api/tow-proposals/$proposalId/reject'),
        headers: headers,
      );
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Falha ao rejeitar proposta: ${response.body}');
      }
    } catch (e) {
  ...
  // Incrementar visualizações da proposta
  static Future<void> incrementViews(String proposalId) async {
    try {
      final headers = await _authHeaders;
      
      await http.post(
        Uri.parse('$baseUrl/api/tow-proposals/$proposalId/views'),
        headers: headers,
      );
    } catch (e) {
      // Não lançar erro para views, apenas log
  ...
  static Future<List<TowProposal>> getExpiringProposals() async {
    try {
      final headers = await _authHeaders;
  ...
  static Future<void> expireEmergencyProposals(String emergencyRequestId) async {
    try {
      final headers = await _authHeaders;
      
      await http.post(
        Uri.parse('$baseUrl/api/tow-proposals/emergency/$emergencyRequestId/expire'),
        headers: headers,
      );
    } catch (e) {
      throw Exception('Erro ao expirar propostas: $e');
    }
  }

  // Obter estatísticas das propostas
  static Future<Map<String, dynamic>> getProposalStats() async {
    try {
      final headers = await _authHeaders;
      
      final response = await http.get(
        Uri.parse('$baseUrl/api/tow-proposals/stats'),
        headers: headers,
      );
  ...
  // Verificar se parceiro pode fazer proposta
