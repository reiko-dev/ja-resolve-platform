...
class SubscriptionService {
  ...
  // Listar assinaturas do parceiro
  static Future<List<Subscription>> getPartnerSubscriptions({SubscriptionStatus? status}) async {
    try {
      final headers = await _authHeaders;
      
      String url = '$baseUrl/api/subscriptions/partner';
      if (status != null) {
  ...
  static Future<Map<String, dynamic>> checkPartnerSubscriptionStatus() async {
    try {
      final headers = await _authHeaders;
      
      final response = await http.get(
        Uri.parse('$baseUrl/api/subscriptions/partner/status'),
        headers: headers,
      );
  ...
  // Obter estatísticas da assinatura
  static Future<Map<String, dynamic>> getSubscriptionStats() async {
    try {
      final headers = await _authHeaders;
      
      final response = await http.get(
        Uri.parse('$baseUrl/api/subscriptions/partner/stats'),
        headers: headers,
      );
  ...
  // Buscar histórico de pagamentos da assinatura
  static Future<List<Map<String, dynamic>>> getPaymentHistory(String subscriptionId) async {
    try {
      final headers = await _authHeaders;
      
      final response = await http.get(
        Uri.parse('$baseUrl/api/subscriptions/$subscriptionId/payments'),
        headers: headers,
      );
  ...
  // Verificar se parceiro pode receber solicitações
  static Future<bool> canReceiveRequests() async {
    try {
      final status = await checkPartnerSubscriptionStatus();
      return status['can_receive'] ?? false;
    } catch (e) {
      return false;
    }
  }

  // Obter benefícios da assinatura
  static List<String> getSubscriptionBenefits(SubscriptionType type) {
    return type.features;
  }

  // Verificar se benefício está disponível
  static bool hasBenefit(SubscriptionType type, String benefit) {
    final benefits = getSubscriptionBenefits(type);
    return benefits.any((b) => b.toLowerCase().contains(benefit.toLowerCase()));
  }

  // Calcular economia anual com assinatura
  static double calculateAnnualSavings(SubscriptionType type) {
  ...
  // Obter próximos benefícios baseados no tipo de parceiro
  static Future<List<String>> getUpcomingBenefits() async {
    try {
      final headers = await _authHeaders;
      
      final response = await http.get(
        Uri.parse('$baseUrl/api/subscriptions/upcoming-benefits'),
        headers: headers,
      );

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final benefitsList = data['data'] as List;
        return benefitsList.cast<String>();
      } else {
        throw Exception('Falha ao buscar próximos benefícios: ${response.body}');
      }
    } catch (e) {
      throw Exception('Erro ao buscar próximos benefícios: $e');
    }
  }
}
