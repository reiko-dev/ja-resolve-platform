...
describe('T01 SAFETY — the migration scope is pinned (no silent drift)', () => {
  test('the exact pinned migration list is 001-008', () => {
  ...
  test('a missing migration file also fails loudly', () => {
    const missing = PINNED_MIGRATIONS.filter((file) => file !== '003_mvp01_tow_foundation.js');
    expect(() => assertPinnedMigrations(PINNED_MIGRATIONS, missing))
